...
class DAG(ds.Graph):
    ...
    def solve(self):
        ...
        result = [[[0, []] for _ in range(self.scc_size)] for _ in range(self.scc_size)]
        for i in range(self.scc_size):
            result[i][i][0] = self.sccs[i].value
            result[i][i][1].append([self.sccs[i].index])

        for m in range(1, self.scc_size):
            resultSpan = list(range(m))
            resultSpan.reverse()
            ...
            for i in resultSpan:
                ...
                for k in range(i + 1, m + 1):
                    option = copy.deepcopy(result[k][m][1])
                    if k in self.scc_neighbors[i]:
                        option[0].insert(0, i)
                    else:
                        option.insert(0, [i])
                    if k > i + 1:
                        value = self.get_score(option) + result[i + 1][k - 1][0]
                        option = option + result[i + 1][k - 1][1]
                    else:
                        value = self.get_score(option)
                    if value > maxV:
                        maxV = value
                        bestOption = copy.deepcopy(option)
                result[i][m] = [maxV, bestOption]
        bestOption = result[0][self.scc_size - 1][1]
        bestVs = []

        for path in bestOption:
            bestVs.append([self.sccs[scc_index].vertices[0].index for scc_index in path])

        return result[0][self.scc_size - 1][0], bestVs
